...
export default function RequestItems() {
  ...
  const [admin1Approval, setAdmin1Approval] = useState(""); // State for approval selection
  ...
  const handleAdmin1ApprovalChange = (e) => {
    setAdmin1Approval(e.target.value); // Handle approval choice
  };

  const handleSaveChanges = () => {
    // Assuming you're using a PUT request to update Admin1Approval in the backend
    fetch(
      `http://localhost:5075/api/RequestItemsApi/UpdateAdmin1Approval/1${currentItem.id}`,
      {
        method: "PUT",
        headers: {
        ...
        body: JSON.stringify({
          ...currentItem,
          admin1Approval: admin1Approval, // Include the updated admin1Approval value
        }),
      }
    )
      .then((response) => response.json())
      .then((data) => {
        ...
        // Optionally update the UI without fetching all data again
        const updatedItems = items.map((item) =>
          item.id === currentItem.id
            ? { ...item, Admin1: admin1Approval }
            : item
        );
              ...
              <div className="mt-4">
                <div className="grid grid-cols-1 gap-5 mt-5">
                  <select
                    ...
                    value={admin1Approval}
                    onChange={handleAdmin1ApprovalChange}
                  >
